...
def _cloud(n=300, seed=0, dtype=torch.float64):
    ...
    nrm = torch.nn.functional.normalize(torch.randn(1, n, 3, generator=g), dim=-1).to(
        dtype
    )
    drv = torch.nn.functional.normalize(torch.randn(1, 3, generator=g), dim=-1).to(
        dtype
    )
...
def test_softmax_over_points_matches_middle_dim_softmax():
    ...
    assert torch.allclose(
        _softmax_over_points(x, fast=True), torch.softmax(x, dim=1), atol=1e-14
    )
    assert torch.equal(_softmax_over_points(x, fast=False), torch.softmax(x, dim=1))
    x32 = x.float()
    rel = (
        (_softmax_over_points(x32) - torch.softmax(x32, dim=1)).norm()
        / torch.softmax(x32, dim=1).norm()
    ).item()
    assert rel < 1e-6
...
def test_fast_point_softmax_is_exact_in_float64(kw):
    ...
    with torch.no_grad():
        a = fast(
            points=pts, normals=nrm, global_vectors=drv, measure_weights=w, **extra
        )
        b = native(
            points=pts, normals=nrm, global_vectors=drv, measure_weights=w, **extra
        )
...
def test_fast_point_softmax_float32_difference_is_roundoff():
    ...
    torch.manual_seed(0)
    fast = ISLA(
        **_CENTERED, hidden=64, n_layers=3, n_slices=32, fast_point_softmax=True
    ).eval()
    torch.manual_seed(0)
    native = ISLA(
        **_CENTERED, hidden=64, n_layers=3, n_slices=32, fast_point_softmax=False
    ).eval()
    native.load_state_dict(fast.state_dict())
    with torch.no_grad():
        a, b = (
            fast(points=pts, normals=nrm, global_vectors=drv, measure_weights=w),
            native(points=pts, normals=nrm, global_vectors=drv, measure_weights=w),
        )
...
def test_fast_point_softmax_keeps_contracts():
    ...
    torch.manual_seed(0)
    m = (
        ISLA(**_CENTERED, hidden=64, n_layers=3, n_slices=32, fast_point_softmax=True)
        .double()
        .eval()
    )
    ...
    with torch.no_grad():
        ...
        scaled_w = m(
            points=pts, normals=nrm, global_vectors=drv, measure_weights=2.5 * w
        )
