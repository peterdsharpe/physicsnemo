...
REF = dict(hidden=64, n_layers=3, n_slices=32, out_scalars=1, out_vectors=1, geo_checkpoint=True, fast_point_softmax=True)


def _cloud(n=700, batch=1, seed=0, device="cuda", dtype=torch.float32):
    g = torch.Generator().manual_seed(seed)
    pts = (torch.randn(batch, n, 3, generator=g) * torch.tensor([3.0, 2.0, 1.0])).to(device, dtype)
    nrm = torch.nn.functional.normalize(torch.randn(batch, n, 3, generator=g), dim=-1).to(device, dtype)
...
def test_fused_geo_region_matches_eager_region(relative, dtype):
    ...
    mk = lambda *shape: (torch.randn(*shape, generator=g) * 0.1).to("cuda", dtype).requires_grad_()  # noqa: E731
    r, z = mk(b, n, 3), mk(b, s, 3)
    nh = torch.nn.functional.normalize(torch.randn(b, n, 3, generator=g), dim=-1).to("cuda", dtype).requires_grad_()
    ms = torch.nn.functional.normalize(torch.randn(b, s, 3, generator=g), dim=-1).to("cuda", dtype).requires_grad_()
    d = torch.nn.functional.normalize(torch.randn(b, 1, 3, generator=g), dim=-1).to("cuda", dtype).requires_grad_()
    ...
    wts = [torch.randn(b, n, s, generator=g).to("cuda", dtype), torch.randn(b, n, s, generator=g).to("cuda", dtype),
    ...
    o_ref, g_ref = run(_geo_region, None, relative)
...
def test_fused_geo_region_bf16_autocast_forward_matches_eager():
    ...
    d = torch.nn.functional.normalize(torch.randn(b, 1, 3, generator=g), dim=-1).cuda().expand(b, n, 3)
...
def test_fused_geo_kernel_passive_decoder_and_batch():
    ...
    q = dict(query_points=inputs["points"][:, :200] + 0.3, query_normals=inputs["normals"][:, :200])
    ref.zero_grad(); opt.zero_grad()
    a = ref(**inputs, **q)
    b = opt(**inputs, **q)
    assert _rel(b.detach(), a.detach()) < 1e-6
    (a * tgt[:, :200]).sum().backward()
    (b * tgt[:, :200]).sum().backward()
    total = torch.cat([p.grad.flatten() for p in ref.parameters() if p.grad is not None]).norm()
...
def test_fused_geo_kernel_keeps_contracts():
    ...
    torch.manual_seed(0)
    m = ISLA(**REF, geo_kernel="fused", frame_mode="centered", scale_mode="reference_length").cuda().double().eval()
