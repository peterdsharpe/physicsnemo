...
def _rotation(seed: int):
    R, _ = torch.linalg.qr(
        torch.randn(3, 3, dtype=D, generator=torch.Generator().manual_seed(seed))
    )
    if torch.det(R) < 0:
        R[:, 0] = -R[:, 0]
    return R
...
def _sample(b=2, n=200, seed=0):
    ...
    nrm = torch.nn.functional.normalize(
        torch.randn(b, n, 3, dtype=D, generator=g), dim=-1
    )
...
def _model(**kw):
    torch.manual_seed(0)
    return (
        ISLA(hidden=32, n_layers=2, n_slices=8, out_scalars=1, out_vectors=1, **kw)
        .to(D)
        .eval()
    )
    ...
    with torch.no_grad():
        ...
        moved_extra = {
            k: (
                ...
                else (v @ R.T if k.endswith("normals") else v)
            )
        ...
        moved = m(
            ...
            normals=nrm @ R.T,
            measure_weights=w,
            global_vectors=None if gv is None else gv @ R.T,
            global_scalars=gs,
            **moved_extra,
        )
...
def test_zero_global_vectors(frame):
    ...
    with pytest.raises(ValueError, match="n_global_vectors=0"):
        m(
            ...
            normals=nrm,
            measure_weights=w,
            global_vectors=torch.randn(2, 1, 3, dtype=D),
        )
...
def test_two_global_vectors(frame):
    ...
    with torch.no_grad():
        scaled = m(
            ...
            normals=nrm,
            measure_weights=w,
            global_vectors=gv * torch.tensor([[[3.0], [0.2]]], dtype=D),
        )
        swapped = m(
            points=pts, normals=nrm, measure_weights=w, global_vectors=gv.flip(1)
        )
        shared = m(points=pts, normals=nrm, measure_weights=w, global_vectors=gv[:1])
    assert torch.allclose(scaled, base, atol=1e-10)
    assert not torch.allclose(
        swapped, base, atol=1e-3
    )  # the two inputs are distinct channels
    assert torch.allclose(
        shared[:1], base[:1], atol=1e-10
    )  # a single set is shared across the batch
    with pytest.raises(ValueError, match="multiple of 2 vectors"):
        m(
            points=pts,
            normals=nrm,
            measure_weights=w,
            global_vectors=torch.randn(5, 3, dtype=D),
        )
...
def test_two_vectors_passive_and_query_token_paths():
    ...
    gv = torch.randn(2, 2, 3, dtype=D)
    m_passive = _model(n_global_vectors=2, query_independent=True, n_decoder_layers=1)
    _assert_se3(
        m_passive, pts, nrm, w, gv, None, extra=dict(query_points=q, query_normals=qn)
    )
    m_qt = _model(n_global_vectors=2, query_tokens=True, query_mass="source_total")
    _assert_se3(
        m_qt, pts, nrm, w, gv, None, extra=dict(query_points=q, query_normals=qn)
    )
...
def test_global_scalars_condition_every_token():
    ...
    m_passive = _model(n_global_scalars=2, query_independent=True, n_decoder_layers=1)
    _assert_se3(
        m_passive, pts, nrm, w, gv, gs, extra=dict(query_points=q, query_normals=qn)
    )
...
def test_legacy_vector_layouts_still_accepted():
    ...
    with torch.no_grad():
        ...
        b = m(
            ...
            normals=nrm,
            measure_weights=w,
            global_vectors=d[None].expand(2, 3),
        )
        c = m(
            points=pts,
            normals=nrm,
            measure_weights=w,
            global_vectors=d[None, None].expand(2, 1, 3),
        )
...
def test_fused_kernel_refuses_other_vector_counts():
    """The fused Triton region is written for one global vector."""
    for k in (0, 2):
...
def test_boundary_data_reaches_every_interior_path():
    ...
    arms = [
        ...
        (
            ...
            dict(
                query_points=q, query_normals=qn, support_points=q, support_normals=qn
            ),
        ),
    ]
    for m, extra in arms:
        assert (
            m.embed[0].in_features == 2
        )  # boundary scalar + global scalar (no vectors, relative frame)
        base = _assert_se3(
            m, pts, nrm, w, None, gs, extra=dict(extra, boundary_scalars=bsc)
        )
        with torch.no_grad():
            other = m(
                points=pts,
