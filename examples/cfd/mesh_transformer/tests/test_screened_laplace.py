...
def test_driver_smoke_reports_per_split_fidelity(tmp_path) -> None:
    """A zero-step CPU run writes the additive operator-fidelity block.

    The untrained exact-Yukawa-basis arm satisfies the screened equation at
    any coefficient values, so its per-split strong-form residual pins the
    diagnostic's zero; the maximum principle is marked inapplicable for the
    screened operator.
    """
    ...
    from screened_laplace import SPLITS, run_experiment
    ...
    on_disk = json.loads((tmp_path / "yukawa_seed3.json").read_text())
