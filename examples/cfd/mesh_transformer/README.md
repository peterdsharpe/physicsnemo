...
The example is organized by role; modules import each other by flat module
name (a small `_paths.py` shim in each source directory plus
`tests/conftest.py` puts every source directory on `sys.path`, so each script
below remains directly runnable, e.g. `python problems/train.py ...`):

- `models/` — model and baseline definitions: the invariant building blocks
  (`models.py`), learned-BIE and layer-potential decoders
  (`self_consistent_kernel.py`, `layer_potential.py`, `stf_multipole.py`,
  `research_models.py`), Transolver reimplementations (`transolver.py`,
  `transolver_intree.py`), and external adapters (`external_baselines.py`).
- `problems/` — one module per benchmark family plus its trainer: the 2D
  conformal Laplace bank (`conformal_laplace.py`, trained by `train.py`),
  Liouville (`liouville.py`), screened Laplace (`screened_laplace.py`),
  3D Laplace (`laplace3d.py`, `laplace3d_study.py`), mixed boundary
  conditions (`mixed_bc_study.py`), potential flow (`potential_flow.py`),
  the multi-field velocity + Bernoulli-pressure family
  (`euler_bernoulli.py`), the rotational steady-Euler family
  (`euler_rotational.py`), and encoder stress tests (`encoder_stress.py`).
- `studies/` — cross-model study drivers and diagnostics: the executable
  study matrix (`study.py`), paired aggregation (`aggregate.py`), performance
  benchmarking (`benchmark.py`), GLOBE backend sweeps
  (`globe_backend_study.py`), scaling (`scale_study.py`), and spectral
  diagnostics (`spectral_diagnostics.py`).
- `datasets/` — the frozen dataset catalog (`dataset_catalog.py`,
  `generate_datasets.py`, `dataset_train.py`), the verified FEM reference
  solvers (`fem_reference.py` for Laplace/screened,
  `fem_navier_stokes.py` for steady incompressible Navier–Stokes with
  Taylor–Hood P2–P1 and Newton), the multi-field fixed-dataset driver
  (`dataset_train_ns.py`), and the cataloged data itself
  (`star_random_trace/`, `ns_cavity_star/`).
- `metrics.py`, `provenance.py` — shared measurement and provenance
  utilities used across problems and studies.
- `tests/` — the example's pytest suite.
- `results/` — checked-in frozen result JSONs referenced by the book.
- `book/` — the Quarto research book (`quarto render` from inside `book/`).
...
- the explicit physical `reference_length` used for nondimensionalization
  (the benchmark's historical scale gauge; `train.py --gauge intrinsic`
  instead lets the model derive its length from the boundary's
  measure-weighted RMS radius, removing the dataset scale convention as an
  input entirely -- see `tests/test_gauge.py` for the convention-drift
  demonstration this eliminates).
...
```bash
uv run --no-sync python examples/cfd/mesh_transformer/studies/study.py \
...
```bash
checkpoint=outputs/mesh_transformer/study/early/globe_exact/seed-17
checkpoint="$checkpoint/globe_exact_reference.pt"
uv run --no-sync python \
  examples/cfd/mesh_transformer/studies/globe_backend_study.py \
  --checkpoint "$checkpoint" \
  --device cuda \
  --output outputs/mesh_transformer/study/globe_theta_sweep.json
```
...
uv run --no-sync python examples/cfd/mesh_transformer/studies/study.py \
  ...
  --output outputs/mesh_transformer/study/finalists.json
```
...
```bash
uv run --no-sync python examples/cfd/mesh_transformer/problems/train.py \
  ...
  --harmonic-cases 4 \
  --device cuda \
  --output-dir outputs/mesh_transformer/mesh_transformer_seed17
```
...
uv run --no-sync python examples/cfd/mesh_transformer/problems/train.py \
  ...
  --steps 1000 \
  --device cuda \
  --output-dir outputs/mesh_transformer/pair_kernel
```
...
```bash
uv run --no-sync pytest -q examples/cfd/mesh_transformer/tests
```
...
```bash
uv run --no-sync python examples/cfd/mesh_transformer/studies/benchmark.py \
