...
"""Reduce the registered raw-versus-similarity coordinate factorial."""
...
COMPARISONS = {
    "pointwise": ("raw_pointwise", "similarity_pointwise"),
    "hybrid": ("raw_hybrid", "similarity_hybrid"),
}
...
def validate_reports(
    ...
    for report in reports:
        ...
        if key[0] not in study.ARM_SPECS:
            raise ValueError(f"unregistered arm {key[0]}")
        feature_system, loss = study.ARM_SPECS[key[0]]
        if report.get("feature_system") != feature_system:
            raise ValueError(f"report {key} has the wrong feature system")
        if report.get("loss") != loss:
            raise ValueError(f"report {key} has the wrong loss")
    ...
    parameter_counts = {report["parameters"] for report in index.values()}
    if len(parameter_counts) != 1:
        raise ValueError("coordinate arms do not have matched capacity")
    return index
...
def apply_registered_decision(
    ...
    for loss, (baseline, candidate) in COMPARISONS.items():
        guards = {
        ...
        comparisons[loss] = {
            ...
            "representation_claim_earned": (guard_pass and operator_splits_passed >= 2),
        }
    ...
    pointwise_earned = comparisons["pointwise"]["representation_claim_earned"]
    hybrid_earned = comparisons["hybrid"]["representation_claim_earned"]
    if not numerical_sanity:
        verdict = "numerically_unresolved"
    elif pointwise_earned and hybrid_earned:
        verdict = "raw_parameterization_is_principal_cause"
    elif hybrid_earned:
        verdict = "coordinates_and_task_alignment_are_complementary"
    elif pointwise_earned:
        verdict = "similarity_coordinates_help_pointwise_identification_only"
    elif any(
        comparison["operator_splits_passed"] == 1 for comparison in comparisons.values()
    ):
        verdict = "similarity_coordinates_help_one_operator_extreme"
    else:
        verdict = "similarity_coordinates_not_principal"
    return {
        "verdict": verdict,
...
def summarize(
    ...
    for arm in study.ARMS:
        ...
        arms[arm] = {
            "feature_system": study.ARM_SPECS[arm][0],
            "loss": study.ARM_SPECS[arm][1],
