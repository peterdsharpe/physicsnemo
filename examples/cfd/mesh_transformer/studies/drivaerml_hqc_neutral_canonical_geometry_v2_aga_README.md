...
reduced intervention is tested against relative L2 `1e-3` for pressure and
WSS in every case and precision. The coherent full bundle is separately
tested for bitwise identity. These two decision gates produce one of four
registered outcomes: `FULL_AND_DERIVED_PASS`, `FULL_ONLY_PASS`,
...
normalization state, dataset manifest/config, and historical metrics. The
diagnostic also asserts the frozen physical `L_ref=5`, model reference
length `=8`, and effective scale `=40` exactly. It refuses to overwrite
outputs. A valid task creates atomic JSON/NPZ artifacts with SHA-256
sidecars plus `DONE_<jobid>`; every exit creates `STATUS_<jobid>`.
