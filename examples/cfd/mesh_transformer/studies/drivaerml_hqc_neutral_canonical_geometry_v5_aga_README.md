...
float32 coordinates are promoted to float64; raw centroids/areas/normals and
the physical area-weighted center are computed once; the literal registered
formulas `P=(X-c0)/40`, `C=(Craw-c0)/40`, `A=Araw/1600`, and `N=Nraw` are
applied; and each completed field is cast once to float32. Identical bundle
tensors and canonical trace centroids are then supplied symmetrically to the
historical primary and fixed paths in both bfloat16 and float32 contexts.
...
that gate. The byte helper explicitly supports rank-zero encoded center/scale
state as well as array-valued geometry and predictions. These two decision
gates produce one of four registered outcomes:
...
schema-v5 preregistration, prior job 304002 JSON/NPZ, execution source tree,
