...
def _validate_slurm_log_content(
    ...
    try:
        ...
        raise AdjudicationFailure("Slurm log is not UTF-8 text") from error
    _require(
        all(character in "\n\r\t" or ord(character) >= 32 for character in text),
        "Slurm log contains control characters",
    )
    lower = text.lower()
    found = sorted(token for token in FORBIDDEN_KEY_TOKENS if token in lower)
    _require(
        not found,
    ...
    for marker in (
        ...
        _require(marker in text, f"Slurm log is missing completion marker: {marker}")
