...
schema-v2 decision gates and outcome. It cannot independently replay runtime
...
EXPECTED_DIAGNOSTIC_SCRIPT_SHA256 = (
    "2519ad31e6dbf3e198068228b70c69b8c66ea158986c524452169af5a23e0bb3"
)
...
EXPECTED_BUNDLE_MEMBER_SHA256_BY_BASENAME = {
    "drivaerml_hqc_canonical_geometry_diagnostic_v2.py": (
        EXPECTED_DIAGNOSTIC_SCRIPT_SHA256
    ),
    "drivaerml_trace_fixed_query_audit.py": EXPECTED_PRODUCER_SHA256,
    "phase1_hqc_neutral_canonical_geometry_preregistration_v2_2026-07-28.json": (
        "7c2a10796fd430d59746cbe83992dc51cec25c283d9a4abe4b855967e836c2d0"
    ),
    "drivaerml_hqc_neutral_canonical_geometry_v2_aga.sbatch": (
        "c0200f3eb6d9e77eb308dc6e513273e4ffcfcf46f815eb7695bc408919c4cbb4"
    ),
    "drivaerml_hqc_neutral_canonical_geometry_v2_aga_README.md": (
        "39baa78b5a8e2fbb0aa070fb6a44858d38969a622b07eeac7849b5227b60b115"
    ),
    "phase1_hqc_neutral_canonical_geometry_launch_manifest_v2_2026-07-28.json": (
        "d5f49962c655b8335a696e805449520ab09bb3638d2451273e0ee1654c00808b"
    ),
...
def _verify_bundle_manifest(path: Path) -> tuple[str, dict[Path, str]]:
    path = path.resolve()
    _require(path.is_file(), "Copied-bundle manifest is missing")
    ...
    for line_number, line in enumerate(
        ...
        _require(
            member.is_file() and not member.is_symlink(), "Bundle member is missing"
        )
    ...
    actual_members = {
        ...
        if member.is_file() and member.resolve() != path
    }
    _require(
        actual_members == set(recorded),
        "Copied-bundle manifest is not complete",
    )
    for basename, expected_digest in EXPECTED_BUNDLE_MEMBER_SHA256_BY_BASENAME.items():
        matches = [
            digest for member, digest in recorded.items() if member.name == basename
        ]
        _require(
            matches == [expected_digest],
            f"Frozen copied-bundle member differs: {basename}",
        )
...
def _validate_completed_job_markers(
    ...
    _require(
        [member.name for member in done_members] == [expected_done],
        "Copied bundle has no unique matching DONE marker",
    )
    _require(
        [member.name for member in status_members] == [expected_status],
    ...
    _require(
        status_members[0].read_text(encoding="ascii") == "rc=0\ncompleted_units=1/1\n",
        "STATUS marker does not report one completed valid unit",
    )


def _load_npz_unique(path: Path) -> dict[str, np.ndarray]:
    with zipfile.ZipFile(path) as archive_zip:
        members = [info.filename for info in archive_zip.infolist()]
    _require(
...
def _difference(left: np.ndarray, right: np.ndarray) -> dict[str, Any]:
    ...
    return {
        ...
        "exact": bool(left.dtype == right.dtype and np.array_equal(left, right)),
        ...
        for precision in PRECISIONS:
            for mode in MODES:
                for path in ("primary", "fixed", "primary_replay"):
                    for field in FIELDS:
                        expected.add(f"{prefix}__{precision}_{mode}_{path}_{field}")
            for path in PATHS:
                for field in FIELDS:
                    expected.add(f"{prefix}__{precision}_historical_{path}_{field}")
...
def _validate_summary_schema_and_provenance(
    ...
    *,
    ...
    _require(scope["resolution"] == 2500, "Scope resolution differs")
    ...
    slurm_job_id = str(provenance["slurm_job_id"])
    _require(slurm_job_id.isdigit(), "Slurm job ID is missing or malformed")
...
def _validate_prior_replay(
    ...
    ids_exact = bool(np.array_equal(current_ids, prior_ids))
...
def adjudicate(
    *,
    ...
    diagnostic_json = diagnostic_json.resolve()
    diagnostic_npz = diagnostic_npz.resolve()
    prior_diagnostic_npz = prior_diagnostic_npz.resolve()
    ...
    summary = json.loads(diagnostic_json.read_text(encoding="utf-8"))
    _require(
        summary.get("schema_version") == DIAGNOSTIC_SCHEMA_VERSION,
    ...
    _validate_completed_job_markers(
        bundle_members,
        str(summary["provenance"]["slurm_job_id"]),
    )
    ...
    for case in cases:
        ...
        _require(case["resolution"] == 2500, f"{prefix} resolution differs")
        _require_exact_keys(
    ...
    arrays = _load_npz_unique(diagnostic_npz)
    ...
    _require(
        ...
        "Diagnostic NPZ key set differs from schema v2",
    )
...
def main(argv: Sequence[str] | None = None) -> None:
    ...
    _atomic_write_json(args.output_json.resolve(), result)
    print(
        f"{result['status']} outcome={result['decision']['outcome']} "
        f"output={args.output_json.resolve()}",
        flush=True,
    )
