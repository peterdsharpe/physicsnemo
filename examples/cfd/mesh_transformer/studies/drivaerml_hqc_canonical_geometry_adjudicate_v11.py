...
def _validate_slurm_log_content(
    ...
    found = sorted(
        ...
        if token in lower
        or "".join(character for character in token if character.isalnum())
        in normalized
    )
    _require(
        not found,
    ...
    expected_completed_lines = [
        f"COMPLETED_UNITS={index}/4 case={case_id}"
        for index, case_id in enumerate(CASE_IDS, start=1)
    ] + ["COMPLETED_UNITS=1/1 rc=0"]
    observed_completed_lines = [
        line for line in lines if line.startswith("COMPLETED_UNITS=")
    ]
    _require(
        observed_completed_lines == expected_completed_lines,
        "Slurm log completed-unit lines differ",
    )
    _require(
        [line for line in lines if line.startswith("EXIT_CODE=")] == ["EXIT_CODE=0"],
        "Slurm log exit-code lines differ",
    )
    done_prefix = "HQC_NEUTRAL_CANONICAL_GEOMETRY_V5_DONE "
    done_lines = [line for line in lines if line.startswith(done_prefix)]
    _require(len(done_lines) == 1, "Slurm log done-marker lines differ")
    timestamp = done_lines[0][len(done_prefix) :]
    try:
        parsed_timestamp = datetime.fromisoformat(timestamp)
    except ValueError as error:
        raise AdjudicationFailure("Slurm log done timestamp is malformed") from error
    _require(
        parsed_timestamp.tzinfo is not None
        and parsed_timestamp.isoformat() == timestamp,
        "Slurm log done timestamp is not canonical and timezone-aware",
    )
    _require(
        lines[-3:]
        == [
        ...
        "Slurm log final success sequence differs",
    )
...
def _require_runtime_string(value: Any, label: str) -> None:
    _require(
        ...
        and "\n" not in value
        and "\r" not in value
        and not any(token in value.lower() for token in FORBIDDEN_KEY_TOKENS),
...
def _validate_summary_schema_and_provenance(
    ...
    *,
    ...
    _require(
        ...
        and all(type(value) is int and 0 <= value <= 99 for value in capability),
        "CUDA device capability is malformed",
    )
