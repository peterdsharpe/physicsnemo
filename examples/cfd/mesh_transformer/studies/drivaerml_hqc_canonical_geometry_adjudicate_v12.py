...
def _validate_slurm_log_content(
    ...
    done_index = lines.index(done_lines[0])
    completed_index = lines.index("COMPLETED_UNITS=1/1 rc=0")
    exit_index = lines.index("EXIT_CODE=0")
    _require(
        done_index < completed_index < exit_index,
        "Slurm log success markers are out of order",
    )
