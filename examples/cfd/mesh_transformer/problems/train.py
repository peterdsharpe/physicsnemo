...
ModelName = Literal[
    ...
    "mesh_transformer_kernel_singpair_h8",
    "mesh_transformer_single_stream",
    "mesh_transformer_kernel_scalar",
...
# MeshTransformer scale gauge: "explicit" consumes the dataset's declared
# reference_length (historical behavior, bitwise preserved); "intrinsic"
# derives the length from the boundary's measure-weighted RMS radius.
Gauge = Literal["explicit", "intrinsic"]
...
def make_model(
    model_name: ModelName, capacity: str, gauge: Gauge = "explicit"
) -> nn.Module:
    """Construct a benchmark model without silently equalizing unlike costs.

    ``gauge`` selects the MeshTransformer scale gauge: ``"explicit"`` (the
    benchmark default) consumes the dataset's declared ``reference_length``
    exactly as before, while ``"intrinsic"`` drops the declared length and
    lets the model derive it from the boundary's measure-weighted RMS radius
    (unconditionally scale equivariant; nothing external to drift).  The
    knob applies only to MeshTransformer-family arms; baselines keep their
    own conventions and reject a non-default gauge rather than silently
    ignoring it.
    """

    if gauge not in ("explicit", "intrinsic"):
        raise ValueError(f"unknown gauge {gauge!r}")
    mesh_transformer_family = model_name.startswith(
        ("mesh_transformer", "lifted_mesh_transformer")
    )
    if gauge != "explicit" and not mesh_transformer_family:
        raise ValueError(
            f"gauge={gauge!r} applies only to MeshTransformer-family arms, "
            f"got model {model_name!r}"
        )
    reference_length_key = "reference_length" if gauge == "explicit" else None
    _build = partial(build_mesh_transformer, reference_length_key=reference_length_key)
    ...
    if model_name == "mesh_transformer_kernel":
        # Identical schema and capacity settings to "mesh_transformer"; only
        # the query decoder changes (dense kernel basis with exact panel
        # quadrature instead of separable moments).
        try:
            config = CAPACITY_CONFIGS[capacity]
        except KeyError:
            raise ValueError(f"unknown capacity {capacity!r}") from None
        return _build(config, query_decoder="kernel")
    if model_name == "mesh_transformer_kernel_nomlp":
        # Dictionary-only ablation: exact double-layer + polynomial members,
        # no learned smooth MLP members. Probes whether the smooth members
        # cause the freq-OOD degradation seen at longer training.
        try:
            config = CAPACITY_CONFIGS[capacity]
        except KeyError:
            raise ValueError(f"unknown capacity {capacity!r}") from None
        return _build(config, query_decoder="kernel", kernel_mlp_members=0)
    if model_name == "mesh_transformer_kernel_nopoly":
        # Polynomial-off ablation: exact double-layer member plus learned MLP
        # smooth members only. Complements the "_nomlp" arm by testing
        # whether the fixed polynomial members {1, b, a} carry information
        # the learned smooth members do not.
        try:
            config = CAPACITY_CONFIGS[capacity]
        except KeyError:
            raise ValueError(f"unknown capacity {capacity!r}") from None
    ...
    if model_name == "mesh_transformer_kernel_singonly":
        # Singular-only science arm: no polynomial and no MLP smooth members,
        # so the kernel dictionary is the exact double-layer member alone and
        # all remaining capacity lives in the operator-conditioned
    ...
    if model_name == "mesh_transformer_kernel_singpair":
        # Singular-pair science arm: the two exact singular members alone
        # (double layer plus single layer; no polynomial and no MLP smooth
        # members). The single layer is what a pure double-layer dictionary
        # lacks on multiply connected domains -- it carries the net-flux
        # (winding) component, e.g. u = a + b*log r on an annulus, which has
        # zero double-layer representation -- so this arm is the minimal
        # Green-representation-complete dictionary.
        try:
            config = CAPACITY_CONFIGS[capacity]
        except KeyError:
            raise ValueError(f"unknown capacity {capacity!r}") from None
    ...
    if model_name in (
        ...
        # Heads-versus-rank probe arms (see studies/heads_rank_study.py for
        # the pre-registered design): the singpair arm with the attention
        # head/rank split traded at fixed total score capacity
        # heads * (scalar_rank + D * vector_rank) = 4*(12 + 2*4) = 80.  One
        # attention layer's pair-coefficient class depends only on that
        # total (mesh_attention README section 7), so any measured
        # difference between these arms localizes in the per-head value and
        # output structure, which the head count does change.  The rank
        # overrides are absolute values tied to the reference capacity;
        # every other capacity is rejected rather than silently mislabeled
        # as capacity-matched.
        if capacity != "reference":
            raise ValueError(
                f"{model_name!r} is defined only at reference capacity, "
                f"got {capacity!r}"
            )
        head_settings = {
            "mesh_transformer_kernel_singpair_h1": dict(
                heads=1, scalar_rank=48, vector_rank=16
            ),
            "mesh_transformer_kernel_singpair_h8": dict(
                heads=8, scalar_rank=6, vector_rank=2
            ),
        }
        config = replace(CAPACITY_CONFIGS[capacity], **head_settings[model_name])
        return _build(
            config,
            query_decoder="kernel",
            kernel_mlp_members=0,
    ...
    if model_name == "mesh_transformer_single_stream":
        # MEASUREMENT-ONLY control, not a production candidate: the
        # two-stream operator/drive separation is collapsed by re-declaring
        # boundary_value as an operator field (it rides the full nonlinear,
        # biased geometry encoder) and driving the read-out path with a
        # constant unit drive.  Pairs with "mesh_transformer_kernel_singpair"
        # at matched parameters (104,569 vs 104,537 at reference capacity);
        # it forfeits the structural zero-drive and superposition contracts,
        # whose measured violation is the point (see
        # models.SingleStreamFusionControl for the pre-registered
        # hypotheses H1-H3).
        from models import build_single_stream_control
        ...
        return build_single_stream_control(
            config, reference_length_key=reference_length_key
        )
    if model_name == "mesh_transformer_kernel_scalar":
        # Scalar-only encoder ablation of "mesh_transformer_kernel": the
        # encoder carries no oriented (rank-1) state between boundary cells
        # (operator/drive vector channels and the vector attention rank are
        # zero), while the kernel dictionary keeps its full default member
        # set. Boundary normals still reach the model through the decoder's
        # pair invariants and exact double-layer member and through the
        # encoder's scalar Gram invariants of position and normal, so this
        # arm measures exactly what the vector channel machinery adds.
        try:
            config = CAPACITY_CONFIGS[capacity]
        except KeyError:
            raise ValueError(f"unknown capacity {capacity!r}") from None
        config = replace(
            config, operator_vector_dim=0, drive_vector_dim=0, vector_rank=0
        )
        return _build(config, query_decoder="kernel")
    if model_name == "mesh_transformer_kernel_singonly_scalar":
        # Scalar-only encoder combined with the singular-only dictionary:
        # the harshest ablation, where the boundary-to-query read is the
        # exact double-layer member alone and the encoder is purely scalar.
        try:
            config = CAPACITY_CONFIGS[capacity]
        except KeyError:
            raise ValueError(f"unknown capacity {capacity!r}") from None
    ...
    if model_name == "mesh_transformer_kernel_singonly_scalar_wide":
        # Capacity-matched scalar-only control: widened scalar channels
        # (52/76, scalar_rank 18 -> 105,301 params, +1% of the vector-ful
        # singular-only arm's 104,261) disentangle "vector channels carry
        # something" from "the scalar arm is just narrower". If the wide
        # scalar arm recovers the vector-ful numbers, the vector machinery
        # is pure capacity; if the gap persists, oriented inter-cell state
    ...
    if model_name == "lifted_mesh_transformer":
        try:
            ...
            raise ValueError(f"unknown capacity {capacity!r}") from None
        return build_lifted_mesh_transformer(
            config, reference_length_key=reference_length_key
        )
    ...
    if model_name in ("transolver_matched", "transolver_native"):
        from transolver import build_transolver

        # Faithful port of the official Transolver irregular-mesh model on one
        # shared boundary+query token sequence (see transolver.py for the full
        # adaptation contract). "matched" is capacity-matched to the 104,261
        # parameter mesh_transformer_kernel_singonly arm; "native" runs the
        # published PDE-benchmark scale.
        return build_transolver(model_name.removeprefix("transolver_"))
    if model_name in ("transolver_intree_matched", "transolver_intree_native"):
        from transolver_intree import build_transolver_intree

        # The in-tree physicsnemo.models.transolver.Transolver on the
        # identical shared boundary+query token sequence as the ported arms
        # (see transolver_intree.py for the residual differences: slice
        # temperature clamped to [0.5, 5], slice-pooling epsilon 1e-2 vs 1e-5,
        # and no inert placeholder parameter).
        return build_transolver_intree(model_name.removeprefix("transolver_"))
...
def _parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--gauge",
        choices=("explicit", "intrinsic"),
        default=RunConfig.gauge,
        help=(
            "MeshTransformer scale gauge: 'explicit' consumes the dataset's "
            "declared reference_length (historical behavior); 'intrinsic' "
            "derives the length from the boundary's measure-weighted RMS "
            "radius (no dataset scale convention to drift)"
        ),
    )
    ...
    parser.add_argument(
        "--output-dir", type=Path, default=Path("outputs/mesh_transformer")
    )
...
def main() -> None:
    ...
    config = RunConfig(
        ...
        problem=problem,
        gauge=getattr(args, "gauge", RunConfig.gauge),
        steps=args.steps,
    ...
    model = make_model(config.model, config.capacity, config.gauge).to(
    ...
    if args.checkpoint is not None:
        ...
        # The gauge is not visible in the state dict (it adds no parameters),
        # so a mismatch would silently re-interpret the checkpoint under the
        # wrong scale convention -- exactly the drift class the intrinsic
        # gauge exists to eliminate.  Older checkpoints predate the knob and
        # were trained under the explicit gauge.
        checkpoint_run_config = checkpoint.get("run_config") or {}
        checkpoint_gauge = checkpoint_run_config.get("gauge", "explicit")
        if checkpoint_gauge != config.gauge:
            raise ValueError(
                f"checkpoint gauge {checkpoint_gauge!r} does not match "
                f"--gauge {config.gauge!r}"
            )
