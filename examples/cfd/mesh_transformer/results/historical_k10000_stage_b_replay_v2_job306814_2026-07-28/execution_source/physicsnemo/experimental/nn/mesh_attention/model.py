...
class CanonicalSourceGeometry:
    ...
    are not re-derived from ``points``.
    """
        ...
        *,
        ...
        if canonical_source_geometry is None:
            ...
            if not torch.compiler.is_compiling():
                geometry_is_valid = (
                    torch.isfinite(geometric_measures).all()
                    & (geometric_measures > 0.0).all()
                )
            ...
            if carries_measure_weights:
                raise ValueError(
                    "canonical_source_geometry does not yet support boundary "
                    "measure weights; include that contract explicitly before "
                    "combining the two opt-in paths"
                )
