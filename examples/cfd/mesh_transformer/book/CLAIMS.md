...
- **Baseline input scale.** ISLA's recipe consumes the unit freestream
  direction (isla_surface.yaml, forward_kwargs.global_vectors); GeoTransolver (geotransolver_surface.yaml:41)
...
- **ENGINEERING 2026-09-11: ISLA class defaults = frame_mode="relative", scale_mode="total_measure"** (#sec-nb-defaults-relative). center_mode
  default "plain" applies only under frame_mode="centered". Constant-gauge recipe configs (isla_surface_constant_gauge [named isla_surface until 2026-09-15], isla_surface_frame [deleted 2026-09-15], isla_volume_support)
...
- **KERNEL-NEUTRAL (2026-09-15).** The fused geometry kernel is NOT training-neutral on two seeds (fp32 pressure +4.9% seed mean; seed 43 +8.9%, seed 42 +0.8%); geo_kernel='eager' stays the default and the reference; never write that the fused kernel is the default or 'neutral'. Evaluation-time kernel difference 1e-7 (fused vs eager on one checkpoint). Epoch time: the like-for-like same-day pair gives 1.1x (26 vs 29 s); the '1.6x' against 2026-09-10 logs is WITHDRAWN, never write it. Three seeds (2026-09-15): mean ratio 1.025 within 3% but fused spread 11% vs eager 1.8% (seed 44 pair 0.978); per-seed criterion fails on seed 43; FIVE pairs (2026-09-15): ratios 1.008/1.089/0.978/1.002/0.971, mean 1.009; 4 of 5 within 3%; seed 43 the only outlier; neither prereg clause met; DECISION: geo_kernel='eager' stays default, fused stays opt-in; 10k epoch gain 1.03-1.1x; 40k neutrality untested (#sec-nb-kernel-neutral-5seed-verdict).
- **W2-C floor test, ISLA half (2026-09-15, fp32; results/floor20k_2026-09-15.json).** ISLA 20k tokens at 1,260 cases 0.0380/0.0388 → 0.0384 vs
  ...
  cost 2.0x step, 1.9x memory. Write "ISLA is not token-starved at 1,260 cases (5% for 2x tokens)". GT half DONE (2026-09-17; results/floor20k_gt_2026-09-17.json; #sec-nb-w2c-floor-verdict):
  unit-direction GeoTransolver at 20k cells 0.0333/0.0347 → 0.0340 vs its 10k pair 0.0355 (ratio 0.956); GT ÷ ISLA at 20k 0.885 vs 0.878 at 10k
  (velocity 0.917 vs 0.900; wall shear 0.920 vs 0.904). W2-C VERDICT: NOT a shared token floor; write "doubling the cells lowers both by about 5% and
...
- **EQ-PRICE PREREG (2026-09-17; #sec-nb-eqprice-prereg).** DIAGNOSTIC (never a configuration): constant-gauge ISLA + raw_coord_channel (research class, code_globin2) at 435 DrivAerML cars, 192/10k, lr 1e-3, seeds 42/43 (eqp_dr_isla_raw_seed42/43; kernel/eqprice_aga.sbatch; eval kernel/eqprice_eval_aga.sbatch on the sample-frame probe at 10k). Comparators (probe/BENCH instrument): cg 0.0561, T 0.0520, GT 0.0509. Bars: positional info IS the deficit if <= 0.0525; partial 0.0525-0.0550; null >= 0.0550. Prediction: null/low partial. Write the arm as "reads the pose of the car; breaks SE(3) by design; diagnostic only".
- **EQ-PRICE VERDICT (2026-09-17; results/eqprice_reduction_2026-09-17.json; #sec-nb-eqprice-verdict).** cg + raw_coord_channel at 435 cars, probe 10k fp32: 0.0547 (0.0553/0.0540) vs cg 0.0561 (-2.6%); GT/arm 0.931 vs 0.907 (+2.4 pts); density factor 13.2. PARTIAL (low end of the prediction): positional information = ~2.5 of the 4-5 trained-function points; ~2 points = the form. Write "the invariance contracts cost about two and a half points of in-distribution ratio on DrivAerML at the reference size"; the arm is a DIAGNOSTIC (breaks SE(3); density factor 13x), never a configuration.
