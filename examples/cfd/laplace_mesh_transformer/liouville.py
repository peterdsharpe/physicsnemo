...
def pde_residual(
    ...
    *,
    ...
    Computed in float64 at 32 interior points on two in-distribution cases,
    with ``u`` the model's prediction (``reference_length == 1``, so no
    explicit nondimensionalization factor remains).  Scale calibration: the
    exact solution scores ``0`` and any harmonic prediction
    (``lap u == 0``) scores exactly ``2``.
    """
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--model",
        required=True,
        choices=("pair_kernel", "harmonic_panel_bie", "boundary_mean"),
    )
